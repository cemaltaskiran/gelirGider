...
public class AccountantWindow {
     ...
     */
    public AccountantWindow(String username, Connection dbc) {
    ...
    private void initialize() {
        ...
        lblSpendingLogo.setIcon(new ImageIcon(AccountantWindow.class.getResource("/Images/SpendingLogo.png")));
        ...
        buttonLogout.setIcon(new ImageIcon(AccountantWindow.class.getResource("/Images/Buttons/LogoutButton.png")));
        ...
        JLabel lblMuhasebeciBilgileri = new JLabel("Muhasebeci Bilgileri");
        lblMuhasebeciBilgileri.setHorizontalAlignment(SwingConstants.CENTER);
        lblMuhasebeciBilgileri.setFont(new Font("Ubuntu", Font.PLAIN, 22));
        lblMuhasebeciBilgileri.setBounds(10, 11, 280, 45);
        panelInfo.add(lblMuhasebeciBilgileri);

        JTextPane textPane_2 = new JTextPane();
        textPane_2.setText(accInfo);
        ...
        textPane_3.setText(manInfo);
        textPane_3.setFont(new Font("Ubuntu", Font.PLAIN, 12));
        textPane_3.setEditable(false);
        textPane_3.setBackground(SystemColor.menu);
        textPane_3.setAlignmentY(0.0f);
        textPane_3.setAlignmentX(0.0f);
        textPane_3.setBounds(10, 353, 280, 71);
        panelInfo.add(textPane_3);

        JLabel lblAddSpending = new JLabel("\u00D6demeyi Onayla");
        ...
        buttonApply.setIcon(new ImageIcon(AccountantWindow.class.getResource("/Images/applyButton.png")));
        frame.getContentPane().add(buttonApply);

        JButton buttonDeny = new JButton("");
        ...
        buttonDeny.setIcon(new ImageIcon(AccountantWindow.class.getResource("/Images/denyButton.png")));
    ...
    private int calculateLimitLeft() {
        return DBUtils.calculateLimitLeft(username);
    }
    ...
    private void updateTable() {
        ...
        try {
            setTableDefault();
            Statement st = dbc.createStatement();
            String sql = "SELECT s.spending_id, t.s_type_name, s.s_date, s.s_amount, s.user_name  FROM spending s, spendingType t, approvedSpending a "
                    + "WHERE t.s_type_id = s.s_type_id AND s.spending_id = a.spending_id AND s.dep_id = " + DBUtils.getUserDep(username) + " ORDER BY spending_id";
    ...
    boolean applySpending() {
        int reply = JOptionPane.showConfirmDialog(null, "Ödenecektir Onaylıyor Musunuz?", "Onay", JOptionPane.YES_NO_OPTION);
        if (reply == JOptionPane.YES_OPTION) {
            try {
                String sql = "DELETE FROM approvedSpending WHERE spending_id = ? ";
                PreparedStatement ps = dbc.prepareStatement(sql);
                ps.setInt(1, Integer.parseInt((String) table.getValueAt(table.getSelectedRow(), 0)));
                ps.executeUpdate();
                sql = "INSERT INTO paidSpending (spending_id,paid_date) VALUES (?,?)";
                ps = dbc.prepareStatement(sql);
                ps.setInt(1, Integer.parseInt((String) table.getValueAt(table.getSelectedRow(), 0)));
                ps.setDate(2, new java.sql.Date((new Date()).getTime()));
                ps.executeUpdate();
                JOptionPane.showMessageDialog(null, "Harcama Ödemesi gerçekleşmiştir.", "Onay", JOptionPane.INFORMATION_MESSAGE);
                updateTable();
                return true;
            } catch (SQLException e) {
                e.printStackTrace();
